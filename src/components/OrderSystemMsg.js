...
export default function OrderSystemMsg({ orderDetail, phoneNumber,amount,accName,qrCode,handleBrief,message, acceptRequest, approveProgress, setPrice, submitSlip, approveSlip, rejectSlip, editProgress, delProgress, myId, scrollRef }) {
  ...
  const [src, setSrc] = useState("")
    ...
    return (
        <>
            ...
            {message.step_name == "รับคำขอจ้าง" &&
                <Flex gap="small" justify="center" align="center" vertical style={flexSysDialog}>
                    {orderDetail.artist_id == myId ? <p>ยอมรับคำขอแล้ว รอส่งภาพร่างให้ลูกค้า</p> : <p>นักวาดยอมรับคำขอแล้ว</p>}
                </Flex>}
            ...
            <Modal title="รีวิว" open={IsReviewModalOpen} footer="" onCancel={ReviewModalHandle} style={{ maxWidth: "1000px" }}>
                ...
                <Form
                    layout="vertical">
                    <Flex justify='center'>
                        <Rate tooltips={desc} onChange={setReviewValue} value={reviewValue} allowClear={false}/>
                        {reviewValue ? <span>{desc[reviewValue - 1]}</span> : ''}
                    </Flex>
                    <Form.Item label="ความคิดเห็น" style={{textAlign:"center"}}>
                    ...
                    <Form.Item>
                        <Flex gap="small" justify='flex-end'>
                            ...
                            <Button size="large" type="primary" shape="round">ส่งรีวิว</Button>
                        </Flex>
                    </Form.Item>
                </Form>
                {/* </Flex> */}
