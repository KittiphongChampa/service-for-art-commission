...
const NavbarUser = (props) => {
            ...
            <div className='noti-wrapper' key={keyData} style={{ backgroundColor: read === 'อ่านแล้ว' ? 'white' : '#ebf1ff' }}>
                {action.includes('แอดมิน') ? 
                    (
                        ...
                        //     </span>
                        // </a>
                        <a href={linked} onClick={() => systemNotionClick(keyData, action)}>
                            <div className="pic">
                                <img src="https://as2.ftcdn.net/v2/jpg/04/62/88/97/1000_F_462889752_tSWP7qDYpUIL6QRlbyIC8v68jaXwVXyx.jpg" />
                            </div>
                            <div className="data">
                                {system} {action} <span style={{color:'gray'}}>{data.created_at}</span>
                            </div>
                        </a>
                    )
                : 
    ...
    const systemNotionClick = async (keyData, action) => {
        await axios.put(`${host}/noti/readed/?work_keyData=${keyData}&action=${action}`)
        hasnotread = notifications.some((noti) => noti.noti_read === 0);
    };
